...
public class MqCController {
    ...
    @RabbitListener(queues = QUEUE_DIRECT1)//说明这个类是配置类，这个rabbit监听器监听name 为 tea 的消息队列。若出现多个相同监听，默认轮流处理消息。
    //    @RabbitHandler        //@RabbitListener 标注在类上面表示当有收到消息的时候，就交给 @RabbitHandler 的方法处理(此方式在类下有多个同名重载方法时，会导致循环重试错误，暂未查询到有效解决方法)
    public void directQueue1(Message message, Channel channel) throws IOException {
        log.info("{}: {}", QUEUE_DIRECT1, new String(message.getBody()));
...
//        ( channel.basicNack 与 channel.basicReject 的区别在于basicNack可以拒绝多条消息，而basicReject一次只能拒绝一条消息)
    ...
    public void fanoutQueue2(Message message, Channel channel) throws IOException {
        log.info("{}: {}", QUEUE_FANOUT1, new String(message.getBody()));
    ...
    public void topicQueue1(Message message, Channel channel) throws IOException {
        log.info("{}: {}", QUEUE_TOPIC1, new String(message.getBody()));
        channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
    }
}
