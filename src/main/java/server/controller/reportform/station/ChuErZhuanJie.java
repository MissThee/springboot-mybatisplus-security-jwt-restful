...
@RequestMapping("/reportForm/ChuErZhuanJie")
@RestController("FunChuErZhuanJie")
public class ChuErZhuanJie {
    @Autowired
    FunReportStationDataCezService funReportStationDataCezService;
    ...
    public Res dataEdit(@RequestBody JSONObject bodyJO) {
        ...
        if (funReportStationDataCezService.updateData(reportData)) {
    ...
    public void excel(@RequestBody() JSONObject bodyJO, HttpServletResponse response) throws Exception {
        ...
        String fileName = "楚二站转接综合日报表（" + searchDateStr + "）";
...
//行分隔
    ...
    private List<ReportStationDataCez_Res> getData(JSONObject bodyJO) throws ParseException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String searchDateStr = sdf.format(searchDate == null ? new Date() : searchDate);
        return funReportStationDataCezService.selectReportData(searchDateStr, getColumnMap());
    }


    @Autowired
    FunReportStationDataCez0Service funReportStationDataCez0Service;
    ...
    public Res formDataEdit(@RequestBody JSONObject bodyJO) {
        ...
        if (funReportStationDataCez0Service.updateData(reportData)) {
            return Res.successMsg("修改成功");
        } else {
            return Res.failureMsg("修改失败");
        }
    ...
    private List<ReportStationDataCez0_Res> getFormData(JSONObject bodyJO) {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String searchDateStr = sdf.format(searchDate == null ? new Date() : searchDate);
        return funReportStationDataCez0Service.selectReportForm(searchDateStr, getFormColumnMap());
    }
}
